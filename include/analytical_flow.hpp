...
inline float2 taylor_green_velocity(float x, float y)
{
    return {
...
inline float2 taylor_green_velocity(const float2& p){
    return taylor_green_velocity(p.x, p.y);
}

/*  ω = 4π sin(2πx) sin(2πy) */
inline float taylor_green_vorticity(float x, float y)
{
    return 4.0f * static_cast<float>(M_PI) *
           std::sin(TWO_PI * x) * std::sin(TWO_PI * y);
}
inline float taylor_green_vorticity(const float2& p){
