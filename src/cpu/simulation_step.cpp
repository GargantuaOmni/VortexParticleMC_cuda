...
void Simulation::step_cpu(bool periodic)
{
    particles_.pos_temp.resize(particles_.N_cur);
    std::cout << particles_.pos_temp.size() << "size : " << particles_.pos.size()   << std::endl;
    ...
    for(int i = 0; i < particles_.N_cur; ++i)
    {
        ...
        particles_.vel[i]       =  vpos.main   - vneg.main;
        ...
        particles_.pos_temp[i]    += particles_.vel[i]  * P_.dt;
